...
export function RouteEvaluationCard({ evaluation, id, t }: { evaluation: RouteEvaluation; id?: string; t: Translation }) {
  ...
  return <section className="route-evaluation" id={id} aria-labelledby="route-evaluation-title">
    ...
    <article className="route-winner"><div className="route-winner-title"><span className="route-status route-status-recommended">{t.routeEvaluation.recommended}</span><h4>{recommended.title}</h4></div><p>{recommended.summary}</p><div className="route-tool-sequence" aria-label={t.routeEvaluation.tools}>{recommended.toolSequence.map((tool) => <span key={tool}>{tool}</span>)}</div><details className="route-details"><summary className="route-evidence-toggle"><span>{t.routeEvaluation.showEvidence}</span><ChevronDown aria-hidden="true" size={17} /></summary><div className="route-details-content"><div className="route-score-grid">{criteria.map((criterion) => { const item = recommended.scoreBreakdown.find((score) => score.criterion === criterion); return item ? <div key={criterion} className="route-score-item"><div><span>{label(criterion)}</span><strong>{item.score}/100</strong></div><progress max="100" value={item.score} aria-label={`${label(criterion)} ${item.score}/100`} /><p>{item.rationale}</p></div> : null; })}</div><div className="route-evidence-grid">{recommended.scoreBreakdown.map((item) => <div key={item.criterion}><strong>{label(item.criterion)}</strong><ul>{item.evidence.map((evidence) => <li key={`${evidence.source}-${evidence.statement}`}>{evidence.statement}</li>)}</ul></div>)}</div><div className="route-strengths"><div><strong>{t.routeEvaluation.strengths}</strong><ul>{recommended.strengths.map((item) => <li key={item}>{item}</li>)}</ul></div><div><strong>{t.routeEvaluation.tradeoffs}</strong><ul>{recommended.tradeoffs.map((item) => <li key={item}>{item}</li>)}</ul></div></div></div></details></article>
